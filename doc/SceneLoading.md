...
- `sample_colmap_fibonacci_sphere_points(...)` builds optional shell points around the arithmetic mean of the aligned COLMAP camera centers. The viewer appends those points to point-cloud initializers before nearest-neighbor scale estimation, so the shell inherits overlapping gaussian sizes from its own Fibonacci spacing.
