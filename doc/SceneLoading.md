...
- `sample_colmap_fibonacci_sphere_points(...)` builds optional shell points around the arithmetic mean of the aligned COLMAP camera centers, resolves the shell radius from a UI multiplier times the max aligned COLMAP point distance from that center, and applies deterministic radial jitter up to 10% per point to reduce view-dependent ordering aliasing. The viewer still appends those points to point-cloud initializers, and the appended shell gets its own equal-area overlap scale derived only from `(shell radius, shell point count)` instead of inheriting the sparse-cloud nearest-neighbor scale coefficient.
