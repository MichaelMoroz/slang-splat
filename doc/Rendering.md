...
- Grad-variance mode reads per-splat `(sum, sumSq, count)` gradient norm statistics accumulated before ADAM over the current pre-refinement interval, computes `E(x^2) - E(x)^2`, and colors by its square root using the grad-norm threshold band.
