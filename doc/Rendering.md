...
- For exact conics (undistorted pinhole cameras) the per-scanline span solve shrinks each tile box by half a pixel per side (`centerTightenPx` in `compute_scanline_tile_span_universal`): only pixel centers sample a splat, so a tile whose pixel-center box misses the conic region can never light up. This drops the measured 2-15% of dead `(tile, splat)` pairs with bit-identical output. Equirectangular and distorted cameras keep the conservative full tile box because their conics are fitted, not exact.
...
- Fixed mode flushes quantized gradients per hit with per-thread int atomics (hardware-aggregated). Float mode instead stages wave-reduced gradient vectors, the gradient-norm statistic, and the quantized contribution in groupshared per batch splat (`gsGradStage`/`gsContribStage`) and flushes one atomic set per splat per group at batch end, with the wave reductions hoisted into uniform control flow - measured 1.27-1.36x faster than per-hit wave flushing at 1080p.
- The host selects `csRasterizeBackwardFloat` or `csRasterizeBackwardFloatNoDepth` by `cached_raster_grad_include_depth`; the no-depth variant (the default config) also skips the wave reductions of the four depth-gated components (`ro.z`, `sigma.zz/xz/yz`) that the flush would discard anyway.
