...
7. Run the optimizer pipeline:
  ...
  - `csProjectGaussianParams` applies the remaining Gaussian-specific post-step projection: quaternion normalization, anisotropy clamp, and a camera-aware upper screen-space size clamp driven by the full projected support-ellipse area already produced by the projection pass.
