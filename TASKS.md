...
STATUS: Done
Description on completion: Split the fixed-count training path into explicit forward and backward stages across both the loss stage and the raster stage. `shaders/renderer/gaussian_training_stage.slang` now has separate `csComputeL1LossForward` and `csComputeL1LossBackward` kernels, while `shaders/renderer/gaussian_raster_stage.slang` now uses `csRasterizeTrainingForward` to render and cache per-pixel forward state and `csRasterizeBackward` to consume that cache in a pure backward pass. `GaussianTrainer` dispatches the full path as `raster forward/cache -> loss forward -> loss backward -> raster backward -> optimizer`, and the training tests validate that scalar metrics appear only in the forward pass while parameter gradients appear only after the backward pass. Verified with `pytest tests/test_training_kernels.py tests/test_training_cli_smoke.py tests/test_viewer_presenter.py tests/test_renderer_pipeline.py`.
