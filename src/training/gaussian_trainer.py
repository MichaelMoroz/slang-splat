...
from .schedule import resolve_base_learning_rate, resolve_colorspace_mod, resolve_depth_ratio_weight, resolve_effective_refinement_interval, resolve_image_loss_weights, resolve_learning_rate_scale, resolve_position_lr_mul, resolve_position_random_step_noise_lr, resolve_refinement_clone_budget, resolve_refinement_min_contribution, resolve_max_allowed_density, resolve_sh_band, resolve_sorting_order_dithering, should_run_refinement_step
...
class TrainingHyperParams:
    ...
    depth_ratio_stage1_weight: float = TRAINING_BUILD_ARG_DEFAULTS["depth_ratio_stage1_weight"]; depth_ratio_stage2_weight: float = TRAINING_BUILD_ARG_DEFAULTS["depth_ratio_stage2_weight"]; depth_ratio_stage3_weight: float = TRAINING_BUILD_ARG_DEFAULTS["depth_ratio_stage3_weight"]; ssim_weight_stage1: float = TRAINING_BUILD_ARG_DEFAULTS["ssim_weight_stage1"]; ssim_weight_stage2: float = TRAINING_BUILD_ARG_DEFAULTS["ssim_weight_stage2"]; ssim_weight_stage3: float = TRAINING_BUILD_ARG_DEFAULTS["ssim_weight_stage3"]; high_frequency_weight_stage1: float = TRAINING_BUILD_ARG_DEFAULTS["high_frequency_weight_stage1"]; high_frequency_weight_stage2: float = TRAINING_BUILD_ARG_DEFAULTS["high_frequency_weight_stage2"]; high_frequency_weight_stage3: float = TRAINING_BUILD_ARG_DEFAULTS["high_frequency_weight_stage3"]; low_frequency_weight_stage1: float = TRAINING_BUILD_ARG_DEFAULTS["low_frequency_weight_stage1"]; low_frequency_weight_stage2: float = TRAINING_BUILD_ARG_DEFAULTS["low_frequency_weight_stage2"]; low_frequency_weight_stage3: float = TRAINING_BUILD_ARG_DEFAULTS["low_frequency_weight_stage3"]; max_visible_angle_deg_stage1: float = TRAINING_BUILD_ARG_DEFAULTS["max_visible_angle_deg_stage1"]; max_visible_angle_deg_stage2: float = TRAINING_BUILD_ARG_DEFAULTS["max_visible_angle_deg_stage2"]; max_visible_angle_deg_stage3: float = TRAINING_BUILD_ARG_DEFAULTS["max_visible_angle_deg_stage3"]
    ...
    def __post_init__(self) -> None:
        ...
        self.refinement_momentum_weight_exponent = min(max(float(self.refinement_momentum_weight_exponent), 0.0), 16.0)
        ...
        self.high_frequency_weight = min(max(float(self.high_frequency_weight), 0.0), 1.0)
        self.low_frequency_weight = min(max(float(self.low_frequency_weight), 0.0), 1.0)
        ...
        self.high_frequency_weight_stage1 = min(max(float(self.high_frequency_weight_stage1), 0.0), 1.0)
        self.high_frequency_weight_stage2 = min(max(float(self.high_frequency_weight_stage2), 0.0), 1.0)
        self.high_frequency_weight_stage3 = min(max(float(self.high_frequency_weight_stage3), 0.0), 1.0)
        self.low_frequency_weight_stage1 = min(max(float(self.low_frequency_weight_stage1), 0.0), 1.0)
        self.low_frequency_weight_stage2 = min(max(float(self.low_frequency_weight_stage2), 0.0), 1.0)
        self.low_frequency_weight_stage3 = min(max(float(self.low_frequency_weight_stage3), 0.0), 1.0)
...
class GaussianTrainer:
    ...
    def _refinement_vars(
        ...
        *,
        ...
        return {
            ...
            "g_RefinementMomentumWeightExponent": float(self.training.refinement_momentum_weight_exponent),
    ...
    def _loss_vars(self, frame_index: int, step: int | None = None) -> dict[str, object]:
        resolved_step = self.state.step if step is None else int(step)
        normal_l1_weight, ssim_weight, high_frequency_weight, low_frequency_weight = resolve_image_loss_weights(self.training, resolved_step)
        return {
            ...
            "g_NormalL1Weight": float(normal_l1_weight),
            "g_SSIMWeight": float(ssim_weight),
            "g_HighFrequencyWeight": float(high_frequency_weight),
            "g_LowFrequencyWeight": float(low_frequency_weight),
