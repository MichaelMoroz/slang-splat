...
from ..scene import GaussianScene, build_training_frames, load_colmap_reconstruction, load_gaussian_ply
...
load_colmap_dataset = lambda viewer, root, images_subdir: (lambda recon, xyz_rgb: (_reset_loaded_runtime(viewer), setattr(viewer.s, "colmap_root", Path(root)), setattr(viewer.s, "colmap_recon", recon), setattr(viewer.s, "training_frames", build_training_frames(recon, images_subdir=images_subdir)), setattr(viewer.s, "colmap_point_positions_buffer", viewer.device.create_buffer(size=xyz_rgb[0].shape[0] * 16, usage=spy.BufferUsage.shader_resource | spy.BufferUsage.copy_source | spy.BufferUsage.copy_destination)), setattr(viewer.s, "colmap_point_colors_buffer", viewer.device.create_buffer(size=xyz_rgb[1].shape[0] * 16, usage=spy.BufferUsage.shader_resource | spy.BufferUsage.copy_source | spy.BufferUsage.copy_destination)), viewer.s.colmap_point_positions_buffer.copy_from_numpy(np.pad(xyz_rgb[0], ((0, 0), (0, 1))).astype(np.float32)), viewer.s.colmap_point_colors_buffer.copy_from_numpy(np.pad(xyz_rgb[1], ((0, 0), (0, 1))).astype(np.float32)), setattr(viewer.s, "colmap_point_count", int(xyz_rgb[0].shape[0])), setattr(viewer.s, "scene_path", None), apply_live_params(viewer), viewer.apply_camera_fit(estimate_point_bounds(xyz_rgb[0])), initialize_training_scene(viewer), setattr(viewer.s, "last_error", ""), print(f"Loaded COLMAP: {root} frames={len(viewer.s.training_frames)} images={images_subdir}")))(load_colmap_reconstruction(root), _point_tables(load_colmap_reconstruction(root)))
initialize_training_scene = lambda viewer: None if viewer.s.colmap_recon is None or not viewer.s.training_frames else (lambda init, width, height, renderer, params: (apply_live_params(viewer), setattr(viewer.s, "trainer", GaussianTrainer(device=viewer.device, renderer=renderer, scene=None, scene_count=init.gaussian_count, upload_initial_scene=False, frames=viewer.s.training_frames, adam_hparams=params.adam, stability_hparams=params.stability, training_hparams=params.training, seed=init.seed, init_point_positions_buffer=viewer.s.colmap_point_positions_buffer, init_point_colors_buffer=viewer.s.colmap_point_colors_buffer, init_point_count=viewer.s.colmap_point_count)), viewer.s.trainer.initialize_scene_from_pointcloud(splat_count=init.gaussian_count, init_hparams=init.hparams, seed=init.seed), setattr(viewer.s, "scene", SceneCountProxy(init.gaussian_count)), (lambda enc: (renderer.copy_scene_state_to(enc, viewer.s.renderer), viewer.device.submit_command_buffer(enc.finish())))(viewer.device.create_command_encoder()), setattr(viewer.s, "training_active", False), _invalidate(viewer), setattr(viewer.s, "scene_init_signature", _scene_signature(viewer)), update_debug_frame_slider_range(viewer), _reset_loss_debug(viewer), setattr(viewer.s, "last_error", ""), print(f"Initialized training scene ({init.gaussian_count:,} gaussians)")))(viewer.init_params(), int(viewer.s.training_frames[0].width), int(viewer.s.training_frames[0].height), ensure_renderer(viewer, "training_renderer", int(viewer.s.training_frames[0].width), int(viewer.s.training_frames[0].height), allow_debug_overlays=False), viewer.training_params())
