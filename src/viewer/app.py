...
_TRAINING_PARAM_KEYS = {
    ...
    "mcmc_growth_ratio": "mcmc_growth_ratio",
    "mcmc_position_noise_scale": "mcmc_noise_lr_scale",
...
_TRAINING_DEFAULTS = default_control_values("Train Schedule", "Train Optimizer", "Train Stability", "Train MCMC")
_HIDDEN_TRAINING_DEFAULTS = {
    "scale_l2_weight": TrainingHyperParams().scale_l2_weight,
    "densify_grad_threshold": TrainingHyperParams().densify_grad_threshold,
    "percent_dense": TrainingHyperParams().percent_dense,
    "screen_size_prune_threshold": TrainingHyperParams().screen_size_prune_threshold,
    "world_size_prune_ratio": TrainingHyperParams().world_size_prune_ratio,
    "opacity_reset_interval": TrainingHyperParams().opacity_reset_interval,
}
_training_kwargs = lambda value_for: {**_HIDDEN_TRAINING_DEFAULTS, **{name: value_for(control) for name, control in _TRAINING_PARAM_KEYS.items()}}
...
default_training_params = lambda background=DEFAULT_VIEWER_BACKGROUND: build_training_params(background=background, mcmc_position_noise_enabled=bool(_TRAINING_DEFAULTS["mcmc_pos_noise_enabled"]), mcmc_densify_enabled=bool(_TRAINING_DEFAULTS["mcmc_densify_enabled"]), **_training_kwargs(lambda control: _TRAIN_SETUP_DEFAULTS[control] if control in _TRAIN_SETUP_DEFAULTS else _TRAINING_DEFAULTS[control]))
...
class SplatViewer(spy.AppWindow):
    ...
    renderer_params = lambda self, allow_debug_overlays: RendererParams(radius_scale=float(self.c("radius_scale").value), alpha_cutoff=float(self.c("alpha_cutoff").value), max_splat_steps=int(self.c("max_splat_steps").value), transmittance_threshold=float(self.c("trans_threshold").value), sampled5_safety_scale=float(self.c("sampled5_safety").value), list_capacity_multiplier=self.s.list_capacity_multiplier, max_prepass_memory_mb=self.s.max_prepass_memory_mb, debug_show_ellipses=bool(self.c("debug_ellipse").value) if allow_debug_overlays else False, debug_show_processed_count=bool(self.c("debug_processed_count").value) if allow_debug_overlays else False, debug_show_grad_norm=False)
    ...
    training_params = lambda self: build_training_params(background=self.s.background, mcmc_position_noise_enabled=bool(self.c("mcmc_pos_noise_enabled").value), mcmc_densify_enabled=bool(self.c("mcmc_densify_enabled").value), **_training_kwargs(lambda control: self.c(control).value))
