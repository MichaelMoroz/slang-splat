...
ALL_BACKENDS = ("cuda", "vulkan", "d3d12")
...
def pytest_addoption(parser: pytest.Parser) -> None:
    parser.addoption(
        ...
        action="append",
        default=[],
        help="Test backend(s): cuda, vulkan, d3d12. Can be passed multiple times.",
    )
...
def pytest_generate_tests(metafunc: pytest.Metafunc) -> None:
    if "backend_name" not in metafunc.fixturenames:
        return
    selected = metafunc.config.getoption("backend") or list(ALL_BACKENDS)
