...
def _run_prefix(device: spy.Device, prefix_sum, values: np.ndarray, exclusive: bool) -> tuple[np.ndarray, int]:
    ...
    prefix_sum.scan_uint(enc, src, dst, values.size, total, exclusive=exclusive)
    device.submit_command_buffer(enc.finish())
    device.wait()
    return _read_u32(dst, values.size), int(_read_u32(total, 1)[0])


def _run_prefix_from_count_buffer(device: spy.Device, prefix_sum, values: np.ndarray, exclusive: bool) -> tuple[np.ndarray, int]:
    src = _buffer_u32(device, values)
    dst = _buffer_u32(device, np.zeros((values.size,), dtype=np.uint32))
    total = _buffer_u32(device, np.zeros((1,), dtype=np.uint32))
    count = _buffer_u32(device, np.array([values.size], dtype=np.uint32))
    enc = device.create_command_encoder()
    prefix_sum.scan_uint_from_count_buffer(enc, src, dst, count, 0, values.size, total, exclusive=exclusive)
    device.submit_command_buffer(enc.finish())
    device.wait()
    return _read_u32(dst, values.size), int(_read_u32(total, 1)[0])
...
def test_prefix_sum_matches_numpy(device: spy.Device, prefix_sum, count: int, exclusive: bool) -> None:
    ...
    out, total = _run_prefix(device, prefix_sum, values, exclusive)
...
@pytest.mark.parametrize("exclusive", [False, True])
def test_prefix_sum_from_count_buffer_matches_direct(device: spy.Device, prefix_sum, count: int, exclusive: bool) -> None:
    values = np.random.default_rng(4321 + count + int(exclusive)).integers(0, 17, size=count, dtype=np.uint32)
    direct_out, direct_total = _run_prefix(device, prefix_sum, values, exclusive)
...
@pytest.mark.parametrize("exclusive", [False, True])
def test_prefix_sum_from_count_buffer_matches_direct_large(device: spy.Device, prefix_sum, exclusive: bool) -> None:
